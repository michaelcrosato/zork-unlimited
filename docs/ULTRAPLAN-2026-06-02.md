...
>   §1 author is now reachable without a key in CI. _(A keyed end-to-end run remains
>   the one step that needs an API key.)_
...
The Build-Spec §1 thesis — _an AI can author → compile → validate → play (structured
API) → find → fix → regression-lock a text adventure on a deterministic engine it
cannot corrupt_ — is **proven as an engineering harness and green under `npm run
...
**But the single most load-bearing verb in §1 — that _an AI_ authors — has never run
...
hand-written by the loop's orchestrator (`claude -p`, which _bypasses_ the
...
is reserved-but-unpopulated — a regression reports _that_ it diverged, not _where_.
...
This is a **known, published failure mode**, not a bug to patch away: a _frozen
verifier + frozen content distribution_ provably drives an optimizer toward cosmetic
...
- **Play on memorized classic IF** — TALES (arXiv 2504.14128) and TextQuests
  (2507.23701); TALES _documents_ Zork1 leakage in Claude's thinking traces.
...
**No published system unites all four pillars** — author _and_ compile _and_ run on an
engine the LLM is never the runtime of _and_ play via a structured API _and_
regression-lock — **on guaranteed-uncontaminated content.** RPGBench is the closest
prior art and is the _inverse_ on the key axis: in its simulation task the LLM **is**
the engine; here the LLM is **never** the engine. That intersection is AdventureForge's
defensible reason to exist. The stated goal was a _demo of a capability_; the latent
goal is an _eval the field adopts because no contaminated corpus can be it._
...
- **Reached through D** (demo): the verified self-improvement loop + 156-commit log +
  MCP server + verify-integrity guard _is_ the flagship demo — lowest incremental
...
real frontier model and commit the artifact.** Do _not_ front-load heavy benchmark
...
1. **Confirm the loop stays down** until it has a new goal (it is already stopped; the
   point is _don't relaunch unchanged_).
...
1. **KEYSTONE — replace the hardcoded mock.** At `src/mcp/tools.ts:663`, swap
   ...
   author → validate → play → find → fix → lock cycle on a _fresh_ premise and commit it
   to `ai-runs/` as the **first real-LLM proof artifact, post-cutoff timestamped.** This
   converts the headline claim from _mock-demonstrated_ to _demonstrated_ — the single
...
4. **Add a frontier candidate category to the assessor** (content-depth / real-author),
   scored above the 0.5 saturation floor, so the salvaged loop can _nominate_
...
2. **Wire a real LLM provider into the loop's `content_new` path** so every _new_ pack
...
3. **Add a second tamper detector**: an LLM-judge pass over each cycle's _diff_ asking
   ...
   — insufficient _alone_; DGM faking test logs, 2505.22954, is the cautionary tale).
...
1. **Ship the flagship demo** as the distribution vehicle: a sharp landing page +
   ...
   hook: _"an AI wrote a text-adventure engine, then authored fresh games, found its own
   bugs via blind playtests, fixed them, and locked each with a regression test — here is
   the commit log and the real-model scorecard."_
2. **Turn the loop into a fresh-pack generator** (TextWorld-style parameters: map size,
   ...
   run, then scoring it. This cures the frozen-verifier stall _and_ is the contamination
   control no other IF eval has.
3. **Write the arXiv preprint / tech report**: _a determinism-anchored, AI-authored,
   contamination-free text-adventure benchmark._ Lead with `verify-integrity.ts` (the
   ...
   sentence. Claim _mechanically hard to game_, **not** _un-gameable_.
...
1. **Establish AdventureForge as a public benchmark**: a held-out, rolling set of sealed
   packs generated _after_ model cutoffs (the contamination control TALES proves classic
   IF cannot offer), a containerized reproducible harness, a Jericho/TALES-comparable
   free-text mode alongside the structured-API mode, and a public leaderboard _with
   headroom_ (the SWE-Bench-Pro property that keeps a benchmark interesting).
...
- **Benchmark adoption is winner-take-most and hard** — 7 self-generated packs are not a
  benchmark. Reach B _through_ D+A; let the held-out set accrete; don't front-load curation.
