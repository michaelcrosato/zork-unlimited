...
export const FixLayerSchema = z.enum(["content", "engine_rule", "validator", "test", "hint_text", "quest_structure"]);

/** The closed op vocabulary. Each op is a small, reversible data edit. */
export const PatchOpSchema = z.discriminatedUnion("op", [
  z.object({ op: z.literal("set_meta"), field: z.string().min(1), value: z.union([z.string(), z.number(), z.boolean()]) }).strict(),
  // CYOA narrative/hint edits.
  z.object({ op: z.literal("set_scene_text"), id: z.string().min(1), text: z.string().min(1) }).strict(),
  z.object({ op: z.literal("set_choice_text"), scene: z.string().min(1), choice: z.string().min(1), text: z.string().min(1) }).strict(),
  z.object({ op: z.literal("add_scene_journal_hint"), scene: z.string().min(1), text: z.string().min(1) }).strict(),
  ...
  z
    ...
    .strict(),
  z.object({ op: z.literal("add_room_journal_hint"), room: z.string().min(1), text: z.string().min(1) }).strict(),
]);
...
type AnyPack = {
  ...
  scenes?: { id: string; text?: string; on_enter?: unknown[]; choices?: { id: string; text?: string }[] }[];
...
export function applyContentPatch(rawPack: unknown, proposal: ContentPatchProposal): ApplyResult {
  ...
  if (!parsedProposal.success) {
    return { ok: false, report: makeReport("patch", [{ severity: "error", code: "PATCH_INVALID", message: parsedProposal.error.message, where: ["proposal"] }]) };
  }
  const pack = clone(rawPack) as AnyPack;
  const fail = (code: string, message: string, where: string[]): ApplyResult => ({ ok: false, report: makeReport(String(pack.meta?.["id"] ?? "patch"), [{ severity: "error", code, message, where }]) });
  ...
  for (const op of parsedProposal.data.ops) {
    switch (op.op) {
      ...
      case "set_choice_text": {
        const choice = pack.scenes?.find((s) => s.id === op.scene)?.choices?.find((c) => c.id === op.choice);
        if (!choice) return fail("PATCH_TARGET_MISSING", `no choice "${op.choice}" in scene "${op.scene}".`, [`scene:${op.scene}`]);
        choice.text = op.text;
        break;
      }
      case "add_scene_journal_hint": {
        const scene = pack.scenes?.find((s) => s.id === op.scene);
        if (!scene) return fail("PATCH_TARGET_MISSING", `no scene "${op.scene}".`, [`scene:${op.scene}`]);
      ...
      case "add_room_journal_hint": {
        ...
        if (!room) return fail("PATCH_TARGET_MISSING", `no room "${op.room}".`, [`room:${op.room}`]);
  ...
  if (!reparsed.success) {
    const findings = reparsed.error.issues.map((i) => ({ severity: "error" as const, code: "PATCH_SCHEMA_BREAK", message: `${i.message} (${i.path.join(".") || "<root>"})`, where: [i.path.join(".") || "<root>"] }));
    return { ok: false, report: makeReport(String(pack.meta?.["id"] ?? "patch"), findings) };
  }
  const report = parsedProposal.data.mode === "cyoa" ? validateCyoa(reparsed.data as never) : validateParser(reparsed.data as never);
 ...
 */
export function proposeFix(diagnosis: Diagnosis, ctx: { mode: "cyoa" | "parser"; location?: string }): ContentPatchProposal {
  if (diagnosis.type === "soft_lock" && ctx.mode === "parser" && ctx.location) {
    return {
      ...
      ops: [{ op: "add_room_journal_hint", room: ctx.location, text: "You sense there is still a way forward here — look closer at what you carry and what stands before you." }],
    };
  }
  if (diagnosis.type === "soft_lock" && ctx.mode === "cyoa" && ctx.location) {
    return {
      ...
      ops: [{ op: "add_scene_journal_hint", scene: ctx.location, text: "There must be another way on from here." }],
    };
  }
