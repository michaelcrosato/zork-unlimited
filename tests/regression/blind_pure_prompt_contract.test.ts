...
describe("pure blind prompt + runner contract", () => {
  ...
  it("contains transport and game-owned exit instructions without guided coverage", () => {
    ...
    expect(prompt).toMatch(/that\s+option's exact `id` at the declared argument/);
    expect(prompt).toMatch(/candidate's\s+consequence\/timing/);
    expect(prompt).toContain("authenticated already-selected departure terms");
    ...
    expect(prompt).toMatch(/need not expand every\s+option/);
