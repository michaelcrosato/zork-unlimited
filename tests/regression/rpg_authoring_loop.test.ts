 ...
 * by the richest validator, not the model (§16); (b) the loop converges to a GREEN RPG
...
describe("rpg authoring loop (bug_0140, §12.2–3, §13 Stage 4)", () => {
  it("the RICHEST validator REJECTS the first attempt with an RPG-specific defect (ENEMY_DEATH_ENDING_UNDECLARED)", async () => {
    ...
    // The defect lives in the Stage-4 layer: validating the SAME pack as a plain parser
    // pack would NOT raise it — proving the richest validator is what decides correctness.
    const parserCodes = validateParser(first.pack)
      .findings.filter((f) => f.severity === "error")
      .map((f) => f.code);
    expect(parserCodes).not.toContain("ENEMY_DEATH_ENDING_UNDECLARED");
  });
  ...
  it("loops against the RPG validator and converges to a GREEN pack", async () => {
    ...
    // It is a genuine RPG pack: enemies + player stats + a skill check, not a bare parser.
