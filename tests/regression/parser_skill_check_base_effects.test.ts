 ...
 *
 * Before this fix the parser/RPG runners returned ONLY the skill-check
 * resolution, silently dropping `it.effects` — so a pack whose sole win gate
 * was set in those base effects validated GREEN (0 findings) yet was
 * unwinnable at runtime. The validator was truthful; the runner lied.
 */
...
function playToEnding(rng: Rng): { state: GameState; flags: string[] } {
  const pack = ParserPackSchema.parse(rawPack);
  const index = indexParserPack(pack);
  const step = makeStep(buildParserRules(index, () => rng));
...
describe("skill_check interactions fire their base effects (parser)", () => {
  ...
  it("base effects precede roll-outcome effects, mirroring CYOA order", () => {
    const pack = ParserPackSchema.parse(rawPack);
    const index = indexParserPack(pack);
    const rules = buildParserRules(index, () => bestRoll);
    const res = rules.resolve(initStateForParserPack(index, 1), {
      type: "USE",
      target: "winch",
    });
...
describe("skill_check interactions fire their base effects (rpg)", () => {
  it("the same shape holds under the RPG runner", () => {
    const pack = RpgPackSchema.parse({ ...rawPack, enemies: [] });
    const index = indexRpgPack(pack);
    const step = makeStep(buildRpgRules(index, () => worstRoll));
    let s = initStateForRpgPack(index, 1);
    const use = step(s, { type: "USE", target: "winch" });
    expect(use.ok).toBe(true);
    s = use.state;
    expect(Object.keys(s.flags)).toContain("gate_up");
    const move = step(s, { type: "MOVE", direction: "north" });
    expect(move.ok).toBe(true);
    expect(move.state.ended).toBe(true);
    expect(move.state.endingId).toBe("out");
  });
});
