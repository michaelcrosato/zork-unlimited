...
export function exactJuneDriveOverrunPredecessor(current: OverworldManifest): OverworldManifest {
  ...
  const june = ally?.options.find((option) => option.id === "albany:ally_june_cattle_first");
  if (!ally || !june) throw new Error("Albany must retain June's cattle-first field-team term.");
  ally.capability = JUNE_DRIVE_OVERRUN_PREDECESSOR_COPY.capability;
  june.preview = JUNE_DRIVE_OVERRUN_PREDECESSOR_COPY.preview;
  june.tradeoff = JUNE_DRIVE_OVERRUN_PREDECESSOR_COPY.tradeoff;
  return predecessor;
}
...
export function exactDroverRouteDriveRecoveryPredecessor(
  ...
  const predecessor = exactJuneDriveOverrunPredecessor(current);
