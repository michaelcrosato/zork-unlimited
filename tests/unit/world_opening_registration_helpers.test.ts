...
describe("opening registration presentation", () => {
  it("projects all four profiles as roleplay-first receipts", () => {
    ...
    expect(Object.keys(prompt).sort()).toEqual(["id", "kind", "message", "options"]);
    expect(prompt.options).toHaveLength(4);
    for (const [index, option] of prompt.options.entries()) {
      const profile = scene.profiles[index]!;
      ...
      expect(option.summary).toEqual({
        ...
        highlights: [
          { label: "Permanent background", value: "Persists after this dispatch." },
          {
            label: "Return promise",
            value: "No active return promise is included in this background.",
          },
          {
            label: "Wolf-Winter fit",
            value: "No additional starting-DEF distinction is stated for this background.",
          },
        ],
  ...
  it("keeps all-or-none trigger authoring in one profile's exact inspected terms", () => {
