...
function fixtureOverworldWithOpeningContactVariants(
  ...
  // this one-quest, no-export fixture catalog. Generic local content remains
  // valid without these optional authored layers.
...
function withStaleAuditFixtureRoot(run: (root: string) => void): void {
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...fixtureOverworldWithOpeningContactVariants(new Set(["stale_fixture"])),
