...
describe("parser object model", () => {
  ...
  it("a closed container hides its contents until opened", () => {
    ...
    expect(buildParserObservation(index, s).visible_objects.map((o) => o.id)).not.toContain("brass_key");
    expect(ids(s)).not.toContain("take_brass_key");
    s = doId(s, "open_stone_coffer");
    expect(buildParserObservation(index, s).visible_objects.map((o) => o.id)).toContain("brass_key");
  ...
  it("a locked container offers neither unlock (no key) nor open until its key is held", () => {
    ...
    s = go(s, ["go_north", "go_up", "take_rope", "go_down", "go_east", "use_rope_on_old_well", "go_down"]); // well_bottom
  ...
  it("a USE puzzle unlocks a previously-locked exit", () => {
    let s = go(initStateForParserPack(index, 1), ["go_north", "go_up", "take_rope", "go_down", "go_east"]); // old_well, holding rope
