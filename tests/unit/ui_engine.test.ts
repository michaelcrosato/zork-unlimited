...
describe("GameSession — RPG-only structured play", () => {
  ...
  it("projects Cade's strategy scorecard identically through the UI and MCP observations", () => {
    ...
    expect(uiView.dialogue?.text).toContain("Reviews choose nothing");
    expect(uiView.dialogue?.text).toContain("PREPARE SUPPORT chooses nothing");
    expect(uiView.dialogue?.text).toContain("Choose HUNT with GO north or RELEASE JUNE");
    expect(uiView.dialogue?.text).toContain("Choose LURE, DRIVE, or FORTIFY in review");
    expect(uiView.dialogue?.text).toContain("One choice permanently closes the rest");
