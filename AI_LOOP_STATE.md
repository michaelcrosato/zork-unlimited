...
- Surface: engine/session continuity metadata reaches full, compact, UI, browser, and resumed embedded-quest views; it names the same persistent record, exact applied imports or none, and quest-local profile, while direct quest starts omit it and GameState/proof hashes remain unchanged.
- Counterfactual: importless, imported, direct-start, launch, resume, and parent-byte-equivalence tests cover the boundary; independent review caught and resolved a resume omission, then found no remaining P0-P2.
...
- Measured result: both players understood the continuous character, with seed 4310 explicitly reasoning about its lower quest-local defenses; this resolves the immediate two-run check, not the recurring cluster at cohort scale. Resume metadata passes deterministic tests but neither blind run exercised a checkpoint resume.
...
- Guard: the feature build passed health at 395 files/3,024 tests plus UI/all packs; the rebased Market/event combination passes 14 focused files/125 tests, both typechecks, lint/format/integrity, and zero-finding crawl `20260721T070212Z`, with full health intentionally left to landing.
