...
| Code host & gate | **GitHub** (`michaelcrosato/zork-unlimited`) with branch protection + **claude-code-action v1 (GA)** for `@claude` mentions and CI autofix | PRs are the unit of review; CI is the objective referee; `@claude` lets the operator turn a plain-English comment into a fix without opening anything else. |
| App hosting / QA surface | **CLI + MCP stdio server** — no web deployment; the engine is headless. QA is done via `npm run health` locally and the blind LLM playtest over the MCP server (`npm run mcp`). The operator QAs by running the interactive CLI (`npm run play`) or approving the blind-playtest transcript attached to each PR. | A non-technical human QAs by running the play CLI or reading the blind-playtest report attached to the PR — no deployed URL required. |
| Database | **None (filesystem content packs)** — all game state is YAML/JSON content packs under `content/`; no external database service is required. The content hash in every save file and trace provides integrity guarantees. | Eliminates a whole class of database infrastructure risk; content is versioned in git alongside the engine. |
| E2E verification | **Blind LLM playtest via MCP** — a fresh subagent with no repo access plays each content pack end-to-end through the `mcp__adventureforge__*` tools and reports its route, step count, clarity, and confusion. The `npm run health` gate (typecheck + tests + validate + playtest) is the CI bar. | A browser-less headless engine needs no browser automation; the MCP blind-playtest is the equivalent human-facing quality gate. |
...
- **Allow:** `npm` scripts, `node`/`ts-node`, `tsc`, `biome lint`, `vitest`, no DB CLI needed (filesystem-only), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
